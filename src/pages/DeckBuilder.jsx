...
export default function DeckBuilder() {
  ...
  function handleCalculate() {
    const deckSize = 60;
    const handSize = 7;
    const probs = cardItems.filter(c => c.selected).map(c => {
      const k = c.count;
  ...
  return (
    <div className="max-w-7xl mx-auto p-6 rounded-lg">
      <section className="space-y-2 mb-6">
        <h1 className="text-3xl font-semibold text-gray-800">Pokémon TCG Deck Builder</h1>
        <p className="text-gray-600">Import your deck list, check live prices, and calculate opening hand odds.</p>
      </section>
      ...
      <div className="flex flex-wrap justify-between items-center mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={selectAll} className="px-4 py-2 border border-indigo-500 text-indigo-500 rounded-lg hover:bg-indigo-50">Select All</button>
          <button onClick={deselectAll} className="px-4 py-2 border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-100">Deselect All</button>
      ...
      <div className="flex gap-4 mb-4">
        <button onClick={handleLoad} className="flex-1 px-6 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600">Load Cards</button>
        <button onClick={handleCalculate} className="flex-1 px-6 py-2 bg-indigo-500 text-white rounded-lg hover:bg-indigo-600">Calculate Odds</button>
      </div>
      ...
      <div ref={gridRef} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-8 gap-4 mb-6">
        {cardItems.map((c, i) => (
          <div key={`${c.set}-${c.num}`} className={`relative bg-white rounded-lg p-2 text-center ${!c.selected ? 'opacity-50' : ''}`}>
            ...
            {c.imgData ? (
              <img src={c.imgData} alt={c.name} className={
                `mx-auto mb-1 max-h-32 object-contain ${c.count === 0 ? 'filter grayscale' : ''}`
              } />
            ) : (
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center" onClick={() => setShowModal(false)}>
          <div className="bg-white rounded-lg p-6 w-full max-w-md relative">
            ...
            <h3 className="text-xl font-semibold mb-4 text-gray-800">Opening Hand Odds</h3>
            ...
            <div className="text-gray-800 font-medium">Any selected card: {(anyProbability * 100).toFixed(1)}%</div>
          </div>
        </div>
      )}
    </div>
